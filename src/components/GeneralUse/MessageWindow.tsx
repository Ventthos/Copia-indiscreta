...
function getImage(type: string){
    const generalRoute = "src/assets/img/"
    switch(type){
        case "warning":
...
export function MessageWindow({title, message, mode, type, onConfirm, onCancel, onRetry}: Props){
    
    return(
        <div className="w-full h-[100dvh] bg-black/40 absolute flex place-content-center items-center z-50">
            <div className={twMerge("w-8/10 lg:w-1/2 p-1", `${getTypeColor(type)}`)}>
                ...
                <div className="bg-white">
                    ...
                    <div className="flex place-content-center items-center gap-3 bg-[#e0e0e0] p-2">
                        {mode == "yesNo" && 
                            <button 
